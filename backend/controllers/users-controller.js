...
const login = async (req, res, next) => {
  ...
  try {
    ...
    const error = new HttpError('Could not log you, check your credentials and try again.', 500);
    return next(error);
  }
  ...
  let token;
  try {
    token = jwt.sign({userId: existingUser.id, email: existingUser.email}, 'supersecret', {expiresIn: '1h'});
  } catch(err) {
